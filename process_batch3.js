...
const filesToProcess = fs.readFileSync('batch3_files_pc.txt', 'utf8').split('\n').filter(f => f.trim());
const filesToProcessMobile = fs.readFileSync('batch3_files_mobile.txt', 'utf8').split('\n').filter(f => f.trim());
const filesToProcessTablet = fs.readFileSync('batch3_files_tablet.txt', 'utf8').split('\n').filter(f => f.trim());
...
console.log(`Processing Batch 2: ${filesToProcessWithLang.length} files...`);
    ...
    try {
        ...
        keysToProcess.forEach(key => {
            ...
            if (content.includes(oldValue)) {
                newContent = newContent.replace(new RegExp(oldValue, 'g'), newValue);
                modified = true;
