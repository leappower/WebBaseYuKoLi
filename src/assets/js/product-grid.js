...
(function (global) {
  ...
  function renderMobile(p) {
    ...
    return (
      ...
      "&category=" +
      encodeURIComponent(p._categoryLabel || p._category || "") +
      '" class="flex items-center justify-center gap-1 bg-primary text-white px-3 py-1.5 rounded-lg font-bold text-[10px] hover:opacity-90 transition-opacity w-full"><span data-i18n="products_card_inquire">询价</span><span class="material-symbols-outlined text-[10px]">arrow_forward</span></a>' +
      '<a href="' +
      link +
      '" class="flex items-center justify-center gap-0.5 text-xs text-primary font-medium hover:underline py-1"><span data-i18n="products_card_view_detail">详情</span><span class="material-symbols-outlined text-xs">arrow_forward</span></a>' +
      "</div>" +
      "</article>"
    );
  }
