...
(function (global) {
  ...
  function validateField(input) {
    ...
    if (input.required && val === "") {
      error = __safe.t("form_validate_required") || "This field is required.";
    } else if (val !== "") {
      if (type === "email" || name.indexOf("email") !== -1) {
        if (!EMAIL_RE.test(val)) error = __safe.t("form_validate_email") || "Please enter a valid email address.";
      } else if (type === "tel" || name.indexOf("phone") !== -1 || name.indexOf("tel") !== -1) {
        if (!PHONE_RE.test(val)) error = __safe.t("form_validate_phone") || "Please enter a valid phone number.";
      }
    }
  ...
  function showFormSuccess(form, onDone) {
    ...
    setTimeout(function () {
      ...
      overlay.innerHTML = [
        CHECKMARK_SVG,
        '<p style="font-weight:700;font-size:1.125rem;color:#16a34a;">' +
          (__safe.t("form_success_title") || "Submitted Successfully!") +
          "</p>",
        '<p style="color:#64748b;font-size:0.875rem;">' +
          (__safe.t("form_success_desc") || "Our team will reach out within 24 hours.") +
          "</p>",
      ].join("");
          ...
          if (calSection) {
            ...
            showFormSuccess(form, function () {
              safeCall("showNotification", [
                __safe.t("form_submit_success") || "Form submitted successfully!",
                "success",
              ]);
            });
          }
